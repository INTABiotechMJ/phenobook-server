					...
					foreach((array) $parcelas as $parcela){
						...
						<div class='item row' data-id='<?= $parcela->id ?>' data-numero="<?= $parcela->numero ?>" id="parcela_<?= $parcela->id ?>">
							<form action="#" method="POST" class="rowForm">
								...
								echo "<div class='field-input main-id'>$parcela->numero</div>";
								echo "</div>";
								...
								foreach((array)$infoEnsayos as $infoEnsayo){
									...
									echo "<div class='field-info col-xs-6'>";
								...
								foreach((array) $variables as $variable){
									$control = Entity::search("Control", "parcela = '$parcela->id' AND ensayo = '$ensayo->id' ORDER BY id DESC");

									if(!$control){
										$value = null;
									}else{
									//Busco el ultimo control de la parcela y la variable ordenado por localStamp (stamp tomada del celu)
										$lastControl = Entity::search("DetalleControl", "control = '$control->id' AND variable = '$variable->id' ORDER BY id DESC");
										if(!empty($lastControl)){
											$value = $lastControl->valor;
										}else{
											$value = null;
										}
									}

									echo "<div class='field-info col-xs-12 col-md-6'>";
